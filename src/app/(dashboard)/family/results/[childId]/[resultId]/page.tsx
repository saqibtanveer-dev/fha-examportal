...
import { fetchChildResultDetailAction } from '@/modules/family/family-results-actions';
...
type Props = {
  params: Promise<{ childId: string; resultId: string }>;
};

export default async function FamilyResultDetailPage({ params }: Props) {
  await requireRole('FAMILY');
  const { childId, resultId } = await params;

  const result = await fetchChildResultDetailAction(childId, resultId);
  if (!result) notFound();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          { label: 'Results', href: '/family/results' },
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <InfoCard icon={<Award className="h-4 w-4 text-muted-foreground" />} label="Score" value={formatMarks(result.obtainedMarks, result.totalMarks)} />
        <InfoCard icon={<TrendingUp className="h-4 w-4 text-muted-foreground" />} label="Percentage" value={formatPercentage(result.percentage)} />
        <InfoCard icon={<Target className="h-4 w-4 text-muted-foreground" />} label="Passing Marks" value={String(Number(result.passingMarks))} />
        <InfoCard icon={<Clock className="h-4 w-4 text-muted-foreground" />} label="Time Taken" value={durationTaken !== null ? formatDuration(durationTaken) : 'N/A'} />
      </div>
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Detailed view of each question, your child&apos;s answer, and the correct answer.
            </p>
          </div>
