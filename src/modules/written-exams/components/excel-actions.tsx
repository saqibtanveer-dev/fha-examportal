        ...
        {state.status === 'preview' && (
          <div className="space-y-4">
            ...
            <DialogFooter className="flex-col gap-2 sm:flex-row sm:gap-0">
              <Button variant="outline" onClick={handleClose} className="min-h-10">
                Cancel
              </Button>
              <Button
                onClick={() => confirmImport(state.result)}
                disabled={state.result.entries.length === 0 || isFinalized}
                className="min-h-10"
              >
                <Upload className="mr-1.5 h-4 w-4" />
                Import {state.result.entries.length} Entries
              </Button>
            </DialogFooter>
          </div>
        )}
          ...
          <div className="space-y-3 py-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium tabular-nums">
                {state.progress}/{state.total} batches
              </span>
            </div>
            <Progress
              value={state.total > 0 ? (state.progress / state.total) * 100 : 0}
              className="h-2"
            />
          ...
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {state.imported} mark entries imported successfully.
              </p>
            </div>
            <Button onClick={handleClose} className="mt-2 min-h-10">
              Done
            </Button>
          </div>
        )}
